...
ngx_set_inherited_sockets(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        if (getsockname(ls[i].fd, ls[i].sockaddr, &ls[i].socklen) == -1) {    // 由fd获取sockaddr
        ...
        len = ngx_sock_ntop(ls[i].sockaddr, ls[i].addr_text.data, len, 1);    // addr_text中存储字符串形式的IP地址
        if (len == 0) {
            return NGX_ERROR;
        }
        ...
        if (getsockopt(ls[i].fd, SOL_SOCKET, SO_RCVBUF, (void *) &ls[i].rcvbuf,   // 获取socket接收缓冲区大小
                       &olen)
            == -1)
        {
        ...
        if (getsockopt(ls[i].fd, SOL_SOCKET, SO_SNDBUF, (void *) &ls[i].sndbuf, // 获取socket发送缓冲区大小
                       &olen)
            == -1)
        {
            ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_socket_errno,
        ...
        ls[i].deferred_accept = 1;      // TCP_DEFER_ACCEPT标志位置1
#endif
    }
