...
struct ngx_http_phase_handler_s {
    /* 
        HTTP框架会调用checker方法，再由checker方法回调handler方法，
        handler方法不会被框架调用，且普通HTTP模块无法重定义checker方法
    */
    ngx_http_phase_handler_pt  checker;
    ngx_http_handler_pt        handler;
    /*
        nxet表示下一个处理阶段中的第1个ngx_http_phase_handler_t处理方法
    */
    ngx_uint_t                 next;
};
...
/* 
    ngx_http_phase_engine_t结构体是所有ngx_http_phase_handler_t组成的数组 
    ngx_http_core_main_conf_t中有这个结构体，流水式处理HTTP请求的关键数据结构
*/
typedef struct {
    /* handlers表示一个请求可能经历的所有ngx_http_handler_pt方法 */
