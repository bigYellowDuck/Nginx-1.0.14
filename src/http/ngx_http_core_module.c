...
ngx_http_handler(ngx_http_request_t *r)
{
    ...
    if (!r->internal) {
        ...
        r->phase_handler = 0; // phase_handler设置为0
    ...
    /* 请求的write_event_handler方法设置为ngx_http_core_run_phases */
    r->write_event_handler = ngx_http_core_run_phases;
    /* 直接调用ngx_http_core_run_phases */
...
/*
  分阶段处理HTTP请求
  分阶段处理请求有可能在某个阶段就会返回
  这时需要ngx_http_request_hander(在ngx_http_process_request中设置的连接的读写方法)
...
/*
  在解析main级别的配置项时，如果发现了server{}配置项，
  就会调用ngx_http_core_server方法(该方法属于ngx_http_core_module模块)，
  在这个方法里会开始解析srv级别的配置项。
*/
static char *
ngx_http_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    /* 创建ngx_http_conf_ctx_t结构体，该结构体属于server块 */
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_http_conf_ctx_t));
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }

    /* cf->ctx为ngx_http_module在cycle->conf_ctx中的位置，即指向main级别的ngx_http_conf_ctx_t结构体 */
    http_ctx = cf->ctx;
    /* server级别的main_conf指向main级别的main_conf */
    ctx->main_conf = http_ctx->main_conf;

    /* the server{}'s srv_conf */
    /* 为srv_conf和loc_conf重新分配指针数组 */
    ...
    /*
      循环调用所有HTTP模块的create_srv_conf和create_loc_conf方法，
      将返回的结构体指针按照模块序号ctx_index保存到上述的srv_conf和loc_conf指针数组中
    */
    ...
    /* 
      获取ngx_http_core_module模块的srv级别的配置项结构体，类型为ngx_http_core_srv_conf_t 
      再把该结构体的ctx赋值为刚刚为server块创建的ngx_http_conf_ctx_t结构体
    */
    cscf = ctx->srv_conf[ngx_http_core_module.ctx_index];
    cscf->ctx = ctx;

    /* 
      获取ngx_http_core_module模块的main级别的配置项结构体，其指向ngx_http_module模块的conf_t指向的main_ conf
      其类型为ngx_http_core_main_conf_t，该结构体中有个存储指针的动态数组servers，每个指针指向ngx_http_core_srv_conf_t结构体的地址
      即一个http{}内含有多少个server{},该数组就有多少个成员
    */
    cmcf = ctx->main_conf[ngx_http_core_module.ctx_index];

    /* 把当前server块的ngx_http_core_srv_conf_t地址添加到servers动态数组 */
    ...
    *cscfp = cscf;
    ...
    /* parse inside server{} 解析当前server块下的全部srv级别的配置项*/
...
/*
  在解析srv级别的配置项时，如果发现了location{}配置项，
  就会回调ngx_http_core_location方法(该方法属于ngx_http_core_module模块)
  在这个方法就会开始解析loc级别的配置项
*/
static char *
ngx_http_core_location(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    /* 创建ngx_http_conf_ctx_t结构体，该结构体属于location块 */
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_http_conf_ctx_t));
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }
    ...
    /* 
      把loc级别的ngx_http_conf_ctx结构体的main_conf和srv_conf分别设置
      为对应的srv级别的main_conf和srv_conf 
    */
    pctx = cf->ctx;
    ...
    /*
      ...
      将返回的结构体指针按照模块序号ctx_index保存到上述的loc_conf指针数组中
    */
    for (i = 0; ngx_modules[i]; i++) {
        if (ngx_modules[i]->type != NGX_HTTP_MODULE) {
            continue;
    ...
    /* 
      ...
      其类型为ngx_http_core_loc_conf_t
      其中的loc_conf成员为一个指针数组，指向所属location内的所有HTTP模块create_loc_conf方法产生的结构体指针
    */
    ...
    /*
      ngx_http_core_loc_conf_t结构体有一个locations双向链表成员
      它会把server块内的location{}连接起来
    */
