...
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    /* 
        把该连接的读事件改为ngx_http_init_request
        写事件改为ngx_http_empty_handler
    */
    rev = c->read;
...
#if (NGX_STAT_STUB)
    (void) ngx_atomic_fetch_add(ngx_stat_reading, 1);   // 正在接收TCP流的连接数加一
#endif
    ...
    /* 
        如果没有数据发送过来则把读事件添加到定时器 
        超时则触发ngx_http_init_request，不过会因为超时而关闭连接
    */
...
ngx_http_init_request(ngx_event_t *rev)
{
    ...
    c->requests++;  // 处理HTTP请求数加一

    /* connection对象的data指针指向ngx_http_connection_t结构体 */
    hc = c->data;
    ...
    /* 最终connection对象指向ngx_http_request_t结构体 */
    c->data = r;
    ...
    rev->handler = ngx_http_process_request_line;       // 把读事件的回调方法设置为ngx_http_process_request_line
    ...
    /*
        创建指针数组以存放所有HTTP模块对该请求可能创建的上下文结构体
    */
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        rc = ngx_http_parse_request_line(r, r->header_in);  // 解析读到的数据

        if (rc == NGX_OK) {     // 接收完整的HTTP请求行
            ...
            */
            ...
            /*
                把读事件的回调函数设为解析头部的ngx_http_process_request_headers
                且直接调用它
            */
        ...
        if (r->header_in->pos == r->header_in->end) {   // 缓冲区用完

            rv = ngx_http_alloc_large_header_buffer(r, 1);  // 分配更大的缓冲区
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_AGAIN) {
            ...
            /*
                接收HTTP命令行和接收HTTP头部都是用header_in
                如果缓冲区用完
            */
        ...
        if (rc == NGX_OK) { // 解析到一行HTTP头部
            ...
            /*
                把解析到的HTTP头部添加到headers_in.headers链表
            */
        ...
        if (rc == NGX_HTTP_PARSE_HEADER_DONE) {     // HTTP头部全部解析完
            ...
            rc = ngx_http_process_request_header(r);    // 检查接收解析出的HTTP头部是否合法

            if (rc != NGX_OK) {
                return;
            }

            ngx_http_process_request(r);    // 处理HTTP请求
...
ngx_http_read_request_header(ngx_http_request_t *r)
{
    ...
    if (n == NGX_AGAIN) {   
        if (!rev->timer_set) {
            cscf = ngx_http_get_module_srv_conf(r, ngx_http_core_module);
            ngx_add_timer(rev, cscf->client_header_timeout);    // 把读事件添加到定时器
        }

        if (ngx_handle_read_event(rev, 0) != NGX_OK) {      // 把都事件添加到epoll
            ngx_http_close_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
            return NGX_ERROR;
        }
...
ngx_http_process_request(ngx_http_request_t *r)
{
    ...
    /* 如果该连接的读事件(rev方法)在定时器，则把它移除 */
    if (c->read->timer_set) {
        ngx_del_timer(c->read);
    }

#if (NGX_STAT_STUB)
    (void) ngx_atomic_fetch_add(ngx_stat_reading, -1);  // 正在读TCP流数加一
    r->stat_reading = 0;
    (void) ngx_atomic_fetch_add(ngx_stat_writing, 1);   // 正在写TCP流数加一
    r->stat_writing = 1;
#endif
    ...
    */
    ...
    /* 设置请求中的read_event_handler方法 */
    r->read_event_handler = ngx_http_block_reading;

    ngx_http_handler(r);        // 检查是否需要内部跳转 调用ngx_http_core_run_phases
...
/*
    ...
    而对于ngx_http_request_handler方法，如果ngx_http_process_request没能处理完请求，
    这个请求上的事件再次被触发，那就将由此方法继续处理了。
*/
static void
ngx_http_request_handler(ngx_event_t *ev)
{
    ...
    if (ev->write) {        // 当前事件可写
        r->write_event_handler(r);  // 调用写事件 即ngx_http_core_run_phases

    } else {
        r->read_event_handler(r);   // 读事件为ngx_http_block_reading
    }
