...
/*
    在配置文件中遇到"http"回调ngx_http_block方法
*/
...
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    /*
        cycle->conf_ctx数组的第7位存储的是ngx_http_module模块的配置项指针，
        这个指针被定义为指向ngx_http_conf_ctx_t结构 
    */
    ...
    */
    ...
    /* 
        ...
        分配ngx_http_conf_ctx_t模块的main_conf指针数组
    */
     ...
     * 分配ngx_http_conf_ctx_t模块的srv_conf指针数组
     */
     ...
     * 分配ngx_http_conf_ctx_t模块的loc_conf指针数组
     */
     ...
     *
     * 调用所有HTTP模块的create_main_conf, create_srv_conf, create_loc_conf方法
     * 并按照ctx_index序号保存到ngx_http_conf_ctx_t中的数组
     */
    ...
    */
    ...
    /*
        ...
        并且合并main，srv，loc级别下的server，location相关配置项
    */
    ...
    */
    ...
    /*
        初始化ngx_http_core_main_conf_t结构体中的phases阶段数组
        每个数组的成员代表一个阶段，该成员的元素是一个ngx_array_t动态数组，
        保存着HTTP模块初始化时添加到当前阶段的处理方法
    */
    ...
    /*
        调用每个HTTP模块的postconfiguration方法
        各HTTP模块通过postconfiguration方法可以把自己的ngx_http_handler_pt添加到
        上述的phases数组，从而添加到HTTP框架
    */
    for (m = 0; ngx_modules[m]; m++) {
        if (ngx_modules[m]->type != NGX_HTTP_MODULE) {
            continue;
    ...
    *cf = pcf;
    ...
    /*
        遍历上述的phases数组，构造ngx_http_core_main_conf_t结构体中的phase_engine变量
    */
    ...
    /*
        ...
        构造监听端口与server间的关联关系，设置新连接事件的回调方法为ngx_http_init_connection
    */
    ...
    ngx_http_module_t *module, ngx_uint_t ctx_index)
{
    ...
    /*
        从ngx_http_core_main_conf_t的servers动态数组中可以获取
        所有的ngx_http_core_srv_conf_t结构体
    */
    cscfp = cmcf->servers.elts;

    /* 这个ctx是在http{}块下的全局ngx_http_conf_t结构体 */
    ctx = (ngx_http_conf_ctx_t *) cf->ctx;
    saved = *ctx;
    rv = NGX_CONF_OK;

    /* 遍历所有的server块下的全局ngx_http_core_srv_conf_t结构体 */
    for (s = 0; s < cmcf->servers.nelts; s++) {

        /* srv_conf将指向所有的HTTP模块产生的server相关的srv级别的配置项 */
        ctx->srv_conf = cscfp[s]->ctx->srv_conf;

        /* 如果该模块实现了merge_srv_conf方法 */
        if (module->merge_srv_conf) {
            /* saved.srv_conf数组对应的是http{}下由create_srv_conf方法创建的结构体*/
        ...
        /* 如果该模块实现了merge_loc_conf方法 */
        if (module->merge_loc_conf) {

            /* loc_conf数组对的是server{}下由create_loc_conf创建的结构体 */
            ctx->loc_conf = cscfp[s]->ctx->loc_conf;

            /* saved.loc_conf数组对应的是http{}下由create_loc_conf方法创建的结构体*/
            ...
            /*
                clcf的类型是ngx_http_core_loc_conf_t
                通过其locations成员关联server块下所有的location块
            */
            clcf = cscfp[s]->ctx->loc_conf[ngx_http_core_module.ctx_index];

            /* 该方法会将server块与其所包含location块下的结构体进行合并 */
    ...
    void **loc_conf, ngx_http_module_t *module, ngx_uint_t ctx_index)
{
         ...
         q = ngx_queue_next(q))
    {
        ...
        /* clcf指向的是loc块下所有模块通过create_loc_conf方法创建的结构体的指针 */
        clcf = lq->exact ? lq->exact : lq->inclusive;
        /* http和server合并而来的结构体指针*/
        ...
        /* 递归调用，因为location块内可以嵌套location块*/
